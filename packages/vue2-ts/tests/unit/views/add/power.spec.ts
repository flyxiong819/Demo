...
describe('power.vue: 表单校验', () => {
  ...
  it('输入合法的账号和名称，有勾选条款', async () => {
    ...
    expect(component.addType).toEqual(EBusinessId.power);
  ...
  it('输入非法的账号和名称', async () => {
    const mockAddData = { accountNo: '@#￥', name: '……%&%……￥' };
    const wrapper = shallowMount(Power);
    const component = wrapper.vm as any;
